...
Datum: 6.11.23
Beschreibung: Tic Tac Toe singleplayer/multiplayer.
*/
...
int main(){
    ...
    char feld[3][3];
    i=0;
    printf("Waehle einen Modus!\n");
    do{                                                     //Singleplyer/Multiplayer Eingabe
        if(i==1){
            printf("Falsche eingabe!\n");
        }
        printf("Singleplayer(0)\nMultiplayer(1)\n");
        scanf("%d", &modus);
        if(modus<0||modus>1){
            i=1;
        }else{
    ...
    while(win==0){
        ...
        while(i==1){
            ...
            if(modus==1){                                       //kontrolliert on Singleplayer oder Multiplayer
                printf("\nP2 Eingabe (y x): ");
                scanf("%d %d", &yP2, &xP2);
                system("cls");
            }else{
